...
export default function UsersTableComponent({ thead, tbody, total }) {
  ...
  useEffect(() => {
    const fetchUserTypes = async () => {
      try {
        const { data } = await axios.get(`${BASE_URL}/usertypes`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
        const clientType = data.find((type) => type.name === "Cliente");
        if (clientType) {
          setValidUserTypeIds(clientType.id);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (validUserTypeIds) {
      const users = tbody.filter(
        (item) => item.userTypeId === validUserTypeIds
      );
      setData(users);
    }
  }, [validUserTypeIds, tbody]);
            ...
            <tr key={item.id || index}>
              ...
              <td title={item.type}>
                <div className="mc-table-icon role">
                  ...
                  <span>
                    {item.userTypeId === "9f056583-75c5-42f8-a112-27a1127a10e9"
                      ? "Cliente"
                      : "Administrador"}
                  </span>
                </div>
              </td>
              <td title={item.taxId}>{item.taxId}</td>
              <td title={item.email}>{item.email}</td>
              <td title={item.pix}>{item.pix}</td>
      ...
      <Modal show={editModal} onHide={() => setEditModal(false)}>
        <div className="mc-user-modal">
          ...
          <Form.Group className="form-group inline">
            ...
            <Form.Select>
              <option value={activeStatusId}>Ativo</option>
              <option value={inactiveStatusId}>Inativo</option>
              <option value={blockedStatusId}>Bloqueado</option>
            </Form.Select>
          </Form.Group>
          <Modal.Footer>
            <ButtonComponent
